  ...
  def db_create_with_user service, user, pass
    ...
    if info
      ...
      when "mysql"
        ...
        if root_user_use_databag
          ::Chef::Log.info ">>> Using data bag to retrieve db root password <<<<<" 
          user_key = node['openstack']['db']['root_user_key']
          super_password = user_password user_key
          ::Chef::Log.info "Retrieving key root from database library file #{user_key}" 
          ::Chef::Log.info "Retrieving password from database library file #{super_password}"
          key_path = node["openstack"]["secret"]["key_path"]
          ::Chef::Log.info "Loading encrypted key at #{key_path} from database"
        else
